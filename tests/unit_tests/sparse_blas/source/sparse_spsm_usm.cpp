...
int test_spsm(sycl::device* dev, sycl::property_list queue_properties,
    ...
    try {
        ...
        CALL_RT_OR_CT(oneapi::mkl::sparse::spsm_buffer_size, main_queue, transpose_val,
        ...
        sycl::event ev_opt;
        CALL_RT_OR_CT(ev_opt = oneapi::mkl::sparse::spsm_optimize, main_queue, transpose_val,
                      alpha_host_or_usm_ptr, A_view, A_handle, x_handle, y_handle, alg, descr,
                      workspace_usm.get(), dependencies);

        CALL_RT_OR_CT(ev_spsm = oneapi::mkl::sparse::spsm, main_queue, transpose_val,
                      alpha_host_or_usm_ptr, A_view, A_handle, x_handle, y_handle, alg, descr,
                      { ev_opt });
        ...
        if (reset_data) {
            ...
            dependencies.push_back(main_queue.memcpy(ia_usm, ia_host.data(),
                                                     ia_host.size() * sizeof(intType), ev_spsm));
            dependencies.push_back(main_queue.memcpy(ja_usm, ja_host.data(),
                                                     ja_host.size() * sizeof(intType), ev_spsm));
            dependencies.push_back(
                main_queue.memcpy(a_usm, a_host.data(), a_host.size() * sizeof(fpType), ev_spsm));
            dependencies.push_back(
                main_queue.memcpy(y_usm, y_host.data(), y_host.size() * sizeof(fpType), ev_spsm));
        ...
        ev_copy = main_queue.memcpy(y_host.data(), y_usm, y_host.size() * sizeof(fpType), ev_spsm);
    }
    ...
    sycl::event ev_release_descr;
    CALL_RT_OR_CT(ev_release_descr = oneapi::mkl::sparse::release_spsm_descr, main_queue, descr,
                  { ev_spsm });
    ev_release_descr.wait_and_throw();
    free_handles(main_queue, { ev_spsm }, A_handle, x_handle, y_handle);
...
class parseSpsmUsmTests : public ::testing::TestWithParam<sycl::device*> {};

TEST_P(parseSpsmUsmTests, RealSinglePrecision) {
    using fpType = float;
...
TEST_P(parseSpsmUsmTests, RealDoublePrecision) {
    using fpType = double;
...
TEST_P(parseSpsmUsmTests, ComplexSinglePrecision) {
...
TEST_P(parseSpsmUsmTests, ComplexDoublePrecision) {
...
INSTANTIATE_TEST_SUITE_P(SparseSpsvUsmTestSuite, parseSpsmUsmTests, testing::ValuesIn(devices),
                         ::DeviceNamePrint());
