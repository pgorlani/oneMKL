...
inline void spsm_buffer_size(backend_selector<backend::BACKEND> selector,
                             ...
                             std::size_t& temp_buffer_size) {
    BACKEND::spsm_buffer_size(selector.get_queue(), opA, opX, alpha, A_view, A_handle, X_handle,
                              Y_handle, alg, spsm_descr, temp_buffer_size);
}

inline void spsm_optimize(backend_selector<backend::BACKEND> selector,
                             oneapi::mkl::transpose opA, oneapi::mkl::transpose opX,
                             const void* alpha, matrix_view A_view,
                             matrix_handle_t A_handle, dense_matrix_handle_t X_handle,
                             dense_matrix_handle_t Y_handle,
                           spsm_alg alg, spsm_descr_t spsm_descr,
                          sycl::buffer<std::uint8_t, 1> workspace) {
    BACKEND::spsm_optimize(selector.get_queue(), opA, opX, alpha, A_view, A_handle, X_handle, Y_handle,
...
inline sycl::event spsm_optimize(backend_selector<backend::BACKEND> selector,
                             ...
                             dense_matrix_handle_t Y_handle,
                                 ...
                                 const std::vector<sycl::event>& dependencies = {}) {
    return BACKEND::spsm_optimize(selector.get_queue(), opA, opX, alpha, A_view, A_handle, X_handle,
                                  Y_handle, alg, spsm_descr, workspace, dependencies);
}

inline sycl::event spsm(backend_selector<backend::BACKEND> selector,
                             oneapi::mkl::transpose opA, oneapi::mkl::transpose opX,
                             const void* alpha, matrix_view A_view,
                             matrix_handle_t A_handle, dense_matrix_handle_t X_handle,
                             dense_matrix_handle_t Y_handle,
                         spsm_alg alg, spsm_descr_t spsm_descr,
                        const std::vector<sycl::event>& dependencies = {}) {
    return BACKEND::spsm(selector.get_queue(), opA, opX, alpha, A_view, A_handle, X_handle, Y_handle,
